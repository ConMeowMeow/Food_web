        ...
        if (user != null) {
            ...
            if (idStr != null) {
                try {
                    ...
                    // Thực hiện xóa địa chỉ theo ID và UserID
                    dao.deleteAddress(addressId, user.getId());
                } catch (Exception e) {
                    System.err.println("Lỗi khi xóa địa chỉ: " + e.getMessage());
                }
            }
        }

        // Thực hiện xong lập tức chuyển hướng về trang Profile, tự động mở tab Địa chỉ
